...
const Avatar: React.FC<AvatarProps> = ({ name, imageSrc, onClick, positionClasses }) => {
  return (
    <div
      className={`absolute ${positionClasses} transform -translate-x-1/2 -translate-y-1/2 cursor-pointer group animate-fade-in-up-then-bob`} // Updated animation class
      onClick={onClick}
    ...
    >
      <img
        src={imageSrc}
        alt={name}
        className="w-22 h-22 md:w-32 md:h-32 rounded-full object-cover shadow-lg transition-all duration-300 group-hover:shadow-xl" // Shadow is already here
      />
